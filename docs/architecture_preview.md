...
2.  **The `runner/state.py` Module:**
    - **Requirement:** This file must initialize a simple SQLite table (`runs` table with columns: `prompt_hash`, `model`, `output`, `status`). Before `engine.py` calls the API, it _must_ check this local DB. This is the only way to satisfy the "Solo-Engineer/Cost-Aware" constraint.
